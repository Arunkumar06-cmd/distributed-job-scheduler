...
pub async fn list(
    ...
    let project = queries::get_project(&state.pool, q.project_id).await?
...
pub async fn get(
    ...
    let batch = queries::get_batch(&state.pool, batch_id).await?
        .ok_or_else(|| AppError::NotFound("batch not found".to_string()))?;
    let project = queries::get_project(&state.pool, batch.project_id).await?
        .ok_or_else(|| AppError::NotFound("project not found".to_string()))?;
    if !queries::user_in_org(&state.pool, auth.user_id, project.org_id).await? {
        return Err(AppError::Forbidden("forbidden".to_string()));
    }
    // Also fetch jobs in batch
    let jobs = queries::list_jobs(&state.pool, Some(batch.queue_id), None, None, None, Some(batch_id), 1000, 0).await?;
