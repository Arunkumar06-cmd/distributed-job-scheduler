...
pub async fn create(
    ...
    let kind = match req.kind.as_deref().or(req.job_type.as_deref()).unwrap_or("immediate") {
    ...
    let job = queries::create_job_with_outbox(&state.pool, params).await.map_err(|e| {
    ...
    if let Some(nats) = &state.nats {
        ...
        let stream_name = format!("JOBS_{}_{}_{}", project.org_id, project.id, queue.id).replace('-', "_");
        let subject_filter = format!("org.{}.proj.{}.queue.{}.*", project.org_id, project.id, queue.id);
        let _ = js.get_stream(&stream_name).await.or_else(|_| {
            let js2 = js.clone();
            let sn = stream_name.clone();
            let sf = subject_filter.clone();
            // We can't await here easily without blocking; spawn
            tokio::spawn(async move {
                let _ = js2.create_stream(async_nats::jetstream::stream::Config {
                    name: sn,
                    subjects: vec![sf],
            ...
            Err(async_nats::error::Error::from(std::io::Error::new(std::io::ErrorKind::Other, "creating stream async")))
        });
    }
...
fn parse_status(s: &str) -> Option<JobStatus> {
    match s.to_uppercase().as_str() {
        ...
        "RETRY_WAIT" => Some(JobStatus::RetryWait),
...
pub async fn list(
    ...
    let jobs = queries::list_jobs(&state.pool, q.queue_id, status, q.priority_min, None, q.batch_id, page_size, offset).await?;
    let total = queries::count_jobs(&state.pool, q.queue_id, status).await?;
...
pub async fn retry(
    ...
    let retried = queries::manual_retry_job(&state.pool, job_id, project.org_id, project.id, queue.id, subject).await?;
...
pub async fn create_batch(
    ...
    if req.jobs.is_empty() || req.jobs.len() > 1000 {
        return Err(AppError::Validation("batch must have 1..1000 jobs".to_string()));
    }
    let queue = queries::get_queue(&state.pool, req.queue_id)
        .await?
        .ok_or_else(|| AppError::NotFound("queue not found".to_string()))?;
    ...
    let batch = queries::create_batch(&state.pool, project.id, req.queue_id, req.name.as_deref().unwrap_or("batch"), req.jobs.len() as i32).await?;
    let mut created = Vec::new();
    for item in req.jobs {
        let priority = item.priority.or(req.priority).unwrap_or(queue.default_priority);
        let subject = ids::nats_subject(&project.org_id, &project.id, &queue.id, priority);
        let params = queries::CreateJobParams {
            queue_id: req.queue_id,
            org_id: project.org_id,
        ...
        match queries::create_job_with_outbox(&state.pool, params).await {
            Ok(j) => created.push(j),
            Err(e) => {
    ...
    let full_batch = queries::get_batch(&state.pool, batch.id).await?.unwrap();
