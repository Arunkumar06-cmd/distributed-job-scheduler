...
async function api(path,o={},token=''){const h={'Content-Type':'application/json',...(o.headers||{})};if(token)h.Authorization=`Bearer ${token}`;const r=await fetch(path,{...o,headers:h});if(!r.ok){if(r.status===401&&token)window.dispatchEvent(new Event('jobflow:session-expired'));const j=await r.json().catch(()=>({}));throw Error(j?.error?.message||j?.message||r.statusText)}return r.status===204?null:r.json()}
