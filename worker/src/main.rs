...
async fn main() -> anyhow::Result<()> {
    ...
    let queues: Vec<(uuid::Uuid, uuid::Uuid, uuid::Uuid)> = sqlx::query_as::<_, (uuid::Uuid, uuid::Uuid, uuid::Uuid)>(
    ...
    for (qid, pid, oid) in &queues {
        ...
        if js.get_stream(&stream_name).await.is_err() {
            match js.create_stream(async_nats::jetstream::stream::Config {
                name: stream_name.clone(),
                subjects: vec![subject.clone()],
                max_messages: 100_000,
                ..Default::default()
            }).await {
                Ok(_) => tracing::info!(stream = %stream_name, subject = %subject, "created stream for queue"),
                Err(e) => tracing::warn!(error = %e, stream = %stream_name, "failed to create stream (may already exist with overlapping subject)"),
            }
        } else {
    ...
    let mut known: std::collections::HashSet<uuid::Uuid> = queues.iter().map(|(qid,_,_)| *qid).collect();
    tokio::spawn(async move {
        ...
        loop {
            ...
            for (qid, pid, oid) in new_queues {
                if known.contains(&qid) { continue; }
                known.insert(qid);
                let subject = format!("org.{oid}.proj.{pid}.queue.{qid}.*");
                let stream_name = format!("JOBS_{}_{}_{}", oid, pid, qid).replace('-', "_");
                ...
                tokio::spawn(async move {
                    ...
                    registry.register(std::sync::Arc::new(worker::handler::EchoHandler)).await;
                    registry.register(std::sync::Arc::new(worker::handler::SleepHandler)).await;
                    registry.register(std::sync::Arc::new(worker::handler::ExternalPaymentHandler)).await;
                    registry.register(std::sync::Arc::new(worker::handler::AlwaysFailHandler{message:"intentional test failure".to_string()})).await;
                    ...
                    let w = db::queries::upsert_worker(&pool_c, &worker_name, "0.1.0", &hostname, cfg_c.worker_concurrency as i32).await.unwrap();
                    let js2 = async_nats::jetstream::new(nats_c);
                    let consumer = worker::consumer::WorkerConsumer::new(pool_c, js2, w.id, worker_name, cfg_c, registry, sd_c);
    ...
    tokio::spawn(async move {
        loop {
            if sd_notify.is_cancelled() { break; }
